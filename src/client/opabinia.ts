...
export class OpabiniaClient extends BaseClient {
  /**
   * Builds `path?query`, omitting the `?` entirely when there are no
   * parameters. Methods whose options are all optional otherwise emit a
   * trailing `?` on every no-argument call.
   */
